...
<html>
	<head>
		...
		<link rel="stylesheet" href="/Public/Home/style/css/profile.css?v=201412231400"/>
		<link rel="stylesheet" href="/Public/Home/style/css/lib/animate.css?v=201412231400"/>
		<link rel="stylesheet" href="/Public/Home/style/css/lib/module.css?v=201412231400"/>		
			...
			<div class="top clearfix">
				<div id="user_msg">
					<div class="name">
						...
						<!--
						<img class="grade_prof" src="" alt="等级显示">
						-->
					</div>
					<div id="need_value"></div>
					<div id="got_value" ></div>
					<img id="grade_value_btn" src="/Public/Home/style/image/grade_value_btn.png" alt="等级显示" >
					<ul id="value_box" >
						<li>当前等级 : <span><?php echo ($info["score"]["lever"]); ?></span></li>
						<li>当前积分 : <span><?php echo ($info["score"]["score"]); ?></span></li>
						<li>升级还需 : <span><?php echo ($info["score"]["need_score"]); ?></span></li>
					</ul>
				</div>
				<div id="user_photo">
					<img id="origin_ph" src="<?php echo ($userinfo["head_img_url"]); ?>" alt="头像" style="display: block;">
					<img id="change_ph" src="<?php echo ($userinfo["head_img_url"]); ?>" alt="头像" style="display: none;">
				</div>
			</div>
			<ul id="middle_nav" class="clearfix">
				<li><a href="/UserCenter">个人资料</a></li>
				<li class="on"><a href="/UserCenter/goods">我发布的商品</a></li>
				<li><a href="/UserCenter/collect">我的收藏</a></li>
				<li><a href="/UserCenter/message">消息中心</a></li>
			</ul>
			<div id="my_products">
				<?php if(($info["selling"]) != ""): ?><div id="onsale_pro">
					<h1>正在出售</h1>
					<?php if(is_array($info["selling"])): $i = 0; $__LIST__ = $info["selling"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="enshr_each">
						<div class="enshr_info">
							<h2>
								<a href="<?php echo U('Home/Goods/goodsDetail?id='.$vo['id']);?>" title="<?php echo ($vo["title"]); ?>"><?php echo ($vo["title"]); ?></a>
							</h2>
							<p><?php echo ($vo["content"]); ?></p>
							<div class="enshr_state">
								<span class="enshrine_it" onclick="sold(<?php echo ($vo["id"]); ?>);">确认售出</span>
								<span class="enshrine_it make_edition" onclick="off_shelf(<?php echo ($vo["id"]); ?>);">删除</span>
								<span class="auto_out"><?php echo ($vo["failTime"]); ?></span>
							</div>
						</div>
						<a href="<?php echo U('Home/Goods/goodsDetail?id='.$vo['id']);?>">
							<img class="enshr_ph" src="<?php echo ($vo["image_url"]); ?>" alt="<?php echo ($vo["title"]); ?>">
						</a>
					</div><?php endforeach; endif; else: echo "" ;endif; ?>					
				</div><?php endif; ?>
				<?php if(($info["selled"]) != ""): ?><div id="sold_out_pro">
					<h1>已售出</h1>
					<?php if(is_array($info["selled"])): $i = 0; $__LIST__ = $info["selled"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="enshr_each">  
						<div class="enshr_info">
							<h2><a href="<?php echo U('Home/Goods/goodsDetail?id='.$vo['id']);?>" title="<?php echo ($vo["title"]); ?>"><?php echo ($vo["title"]); ?></a></h2>
							<p><?php echo ($vo["content"]); ?></p>
							<div class="enshr_state">
								<span class="auto_out">已售出</span>
							</div>
						</div>
						<a href="<?php echo U('Home/Goods/goodsDetail?id='.$vo['id']);?>">
							<img class="enshr_ph" src="<?php echo ($vo["image_url"]); ?>" alt="<?php echo ($vo["title"]); ?>">
				...
				<?php if(($info["timeout"]) != ""): ?><div id="time_out_pro">
					<h1>已过期</h1>
					<?php if(is_array($info["timeout"])): $i = 0; $__LIST__ = $info["timeout"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="enshr_each">  
						<div class="enshr_info">
							<h2><a href="<?php echo U('Home/Goods/goodsDetail?id='.$vo['id']);?>" title="<?php echo ($vo["title"]); ?>"><?php echo ($vo["title"]); ?></a></h2>
							<p><?php echo ($vo["content"]); ?></p>
							<div class="enshr_state">
								<span class="auto_out">商品已失效</span>
							</div>
						</div>
						<a href="<?php echo U('Home/Goods/goodsDetail?id='.$vo['id']);?>">
							<img class="enshr_ph" src="<?php echo ($vo["image_url"]); ?>" alt="<?php echo ($vo["title"]); ?>">
