$(function(){
	...
	$("#btnRegister").click(function(){
		...
		var app_images2 = $("#app_images2 img").attr("src");
		var app_images3 = $("#app_images3 img").attr("src");
		var app_images4 = $("#app_images4 img").attr("src");
		var Screenshot = app_images+","+app_images2+","+app_images3+","+app_images4;
		var check_appname = $(".app_name_text").attr("check_appname");

		if(appName && app_type && app_explain && app_cover && Screenshot && typeof(check_appname) == 'undefined'){
			$.ajax({
				type:"post",
				url: data.create_frist,
				data:"app_name="+appName+"&app_type="+app_type+"&app_explain="+app_explain+"&app_cover="+app_cover+"&app_images="+Screenshot,
	...
	$(".operation-logo .opt input").change(function(){
		...
		if(names[1]!="gif"&&names[1]!="GIF"&&names[1]!="jpg"&&names[1]!="JPG"&&names[1]!="png"&&names[1]!="PNG"&&names[1]!="JPEG") { 
			alert("上传格式不对"); 
			return; 
		} 
	...
	$("#btnSaveAppInfo").click(function(){
		...
		if(appName && app_type && app_explain && app_cover && Screenshot){
			$.ajax({
				type:"post",
				url:data.create_three,
				data:"app_name="+appName+"&app_type="+app_type+"&app_explain="+app_explain+"&app_cover="+app_cover+"&app_images="+Screenshot+"&app_key="+app_key,
				success:function(datas){
					if(datas.ret == 1){
						alert("应用完成");
	...
	window.settings = {
		...
		post_params: {"app_key" : strKey , "did" : did, "dotype":dotype},
